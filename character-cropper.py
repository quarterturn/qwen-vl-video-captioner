...
def build_prompt(num_refs: int, character_name: str, anime_title: Optional[str]) -> str:
    ...
    format_guide = """Respond ONLY with a valid JSON object — no markdown, no ``` fences, no extra text:

{
  "character_found": true or false,
  "overall_confidence": "high" or "medium" or "low",
  "detections": [
    {
      "time_seconds": float (e.g. 0.5, 2.3),
      "bbox_normalized": [x_min, y_min, x_max, y_max] — floats strictly 0.0 to 1.0, tight around the character,
      "confidence": "high" or "medium" or "low",
      "reason": "brief one-sentence explanation"
    }
  ],
  "reason": "overall brief explanation"
}

CRITICAL RULES:
- bbox_normalized: floats strictly 0.0 to 1.0, make the box as tight as possible around the character (head + body).
- Focus on accurate horizontal placement — center the box on the character's main body/face.
- Avoid full-frame boxes [0.0, 0.0, 1.0, 1.0] unless the character truly occupies the entire screen.
- bbox_normalized MUST use floats from 0.0 to 1.0 ONLY. Never output integers like 275 or 999 — divide pixel values by image width/height.
- Make horizontal placement accurate: center the box on the character's face/body horizontally.
- Output at least 10–20 detections if the character appears frequently.
- Sorted by time_seconds.
"""
